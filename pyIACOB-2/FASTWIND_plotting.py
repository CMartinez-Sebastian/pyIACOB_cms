...
def considered_transitions():
    ...
    with open(fastwinddir+'non_repeated_lines_new.txt') as f:
        lines = f.readlines()
    f.close()
    considered_lines = []
    transitions = {}
    ...
    DATA = pd.read_csv(fastwinddir+'LINES_CNOSi_new_coll.dat',delim_whitespace=True,names=\
                               ["i_l","f_l","fine","CW","oscillator_strengths",\
                                "radiative_damping","c7","c8","collisional_damping",\
                                "sum_gi"],on_bad_lines='skip')
    DATA_H = pd.read_csv(fastwinddir+'LINES_HHe_new_coll.dat',delim_whitespace=True,names=\
...
def model_comparation(star_spectrum,E_C,E_N,E_O,model,vmic,vsini,vmac,R,beta=1.5,rv0=0,plot=False):
    pkl_folder=fastwinddir+fastwinddir.split('/')[-2]+'_pkl'
...
def spectrum_generation(model,elems,elems_abundance,VT,wv1=3800,wv2=7000,stp=0.01,\
                        folder_original_pkl=fastwinddir+fastwinddir.split('/')[-2]+'_pkl/original/'):
    '''
...
def synthetic_model(star,model = None,beta = 1.5):
    ...
    abundances = {}
    for elem in elems:
        metallicities = pd.read_csv('../grid_components/abundances_'+elem+'.dat', header=None,delim_whitespace=True)[1]

...
class spec():
    ...
    def fitline(self, line_c, line=None, width=15, tol=150., func='g', iter=3, 
        fw3414=False, info=False, outfit=False, plot=False, lim=None, p0=None,
        normalization=True, sigma_tres=2.0, dEW_iter_MCMC=False, fit_range=None):
        ...
        Parameters
        ----------
        line_c : float or list
            Sets the central wavelenghts of the lines to search and fit. If a 
            list is given, all lines are fitted at the same time; EW is from the 
            line argument or closest to first central wavelength of the list.
            
        line : float, optional
            Line to be studied. If none is given, is set to line_c first value.
            Default is None.
        ...
        func : str, optional
            ...
            'g' Gaussian (default); 'gc' Gaussian covolved with broadening profiles;
            'l' Lorentzian; 'v' Voigt; 'r' Rotational.
        ...
        lim : list of lists, optional
            Limits of the parameters to be fitted. 
            Default is None.
            
        p0 : array_like, optional
            p0 argument from curvefit. Initial guess for the parameters (length N). 
            If None, then the initial values will all be 1 (if the number of parameters
            for the function can be determined using introspection, otherwise a
            ValueError is raised).
            Default is None.
            
        normalization : bool, optional
            If True, it renormalize the studied line with local continuum.
            Default is True.
            
        sigma_tres : float, optional
            Only if normalization is True. Sigma-clipping value for the continuum
            determination
            Default is 2.0.
            
        dEW_iter_MCMC : int, optional
            If given, number of iterations for a MCMC fitting to stimate the error. 
            NOT RECOMENDED
            Default is False.

        fit_range : list
            Limits for the line to be fitted (min and max wavelength). If given,
            it give wave+flux in that range.
            Default is None.
        ...
        The EW in the dictionary is this of the first line in line_c (the one 
        written in 'line_EW_studied')
        ...
        ############ EW error stimation with MCMC (not recomended) ############       
        if dEW_iter_MCMC:
            continuum_ex,EW_MCMC = [],[]
            start = time.time()
            while i<dEW_iter_MCMC:
                try:
...
def gaussian_convolved_general(x,*params,**kwargs):
    '''
    *params : *A,*lam0,sigma,vsini,beta,vmac
    '''
